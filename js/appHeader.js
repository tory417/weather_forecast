...
export const createHeader = (weatherRes) => {
   ...
   cityName.textContent = weatherRes.name;
   ...
   searchBtn.addEventListener('click', async () => {
      ...
      try {
         ...
         if (weather.message) { //если приходит ошибка с сервера (поле message бывает только в случае ошибки)
   ...
   window.addEventListener('click', (e) => { //при клике на пустое место
      if (e.target == searchInput || e.target == searchBtn || e.target == cityChange) {
         return;
      }
      else { // откатываем назад при щелчке вне указанных ранее частях 
         headerCity.innerHTML = '';
